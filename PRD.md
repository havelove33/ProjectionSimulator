...
### S5. 인체공학 감상 품질 평가
- 룸·프로젝터·관객·재질이 셋업된 상태에서, "시감(인지) 평가" 패널을 열면 면별·종합으로 다음을 한 화면에 본다:
  - 평균 면 휘도가 시감 영역(scotopic / mesopic / photopic) 중 어디에 들어가는지
  - 미디어아트 갤러리 권장 휘도 대역(30–100 nit) 대비 부족/적정/과다
  - 균일도(min/avg) 점수와 SMPTE/현장 기준 통과 여부
  - 면 간 휘도 격차(예: 바닥이 벽 대비 너무 어두움) 경고
  - 주변광 대비 화면 휘도 비율(순응 균형)
- 종합 "감상 품질 등급" A/B/C/D를 반환하고, 어떤 항목 때문에 등급이 깎였는지 한 줄 진단을 함께 표시 → 의사결정에 즉시 사용.
...
- **인체공학 시감(visual perception) 평가 — photopic/mesopic 영역, 권장 휘도 대역 비교, 균일도 등급, 주변광 대비, 면 간 격차, 종합 등급 A/B/C/D + 진단 메시지**
- 한국어 UI (영어 토글은 후순위)
...
### FR-12. 인체공학 감상 품질 평가 (Perceptual Quality)
> 광도학 결과(lux/nit)를 사람의 시지각 관점에서 다시 해석하는 별도 모듈. 절대값(밝기)뿐 아니라 **인지된 화질**의 추정치를 산출한다. 수식은 §9.9 참조.

- FR-12.1: **시감 영역 분류 (photopic / mesopic / scotopic)**
  - 면별 평균 휘도 L_avg(nit)에 대해 다음 영역으로 분류
    - photopic ≥ 3 nit (색·디테일 인지 양호)
    - mesopic 0.005 ~ 3 nit (색 약화)
    - scotopic < 0.005 nit (색 미인지, 간상세포)
  - 결과 패널에 색상 배지로 표시
- FR-12.2: **권장 휘도 대역 비교**
  - 콘텐츠 타입 프리셋(미디어아트 갤러리 / 디지털 시네마 / 옥외 디지털 사이니지 / 사용자 정의)에 따른 목표 대역
    - 미디어아트 갤러리(기본): **30–100 nit** (주변 어두움 가정)
    - 디지털 시네마(SMPTE/DCI): **41–55 nit** (어두운 극장)
    - 옥외 사이니지: **300–700 nit** (밝은 환경)
  - 면별로 부족(under) / 적정(in band) / 과다(over)를 표시 + 부족분(nit) / 과다분 수치
- FR-12.3: **균일도(uniformity) 등급**
  - U = E_min / E_avg (또는 L_min / L_avg)에 대해
    - A ≥ 0.75 (영화관급), B 0.6–0.75 (일반 미디어아트), C 0.4–0.6 (수용 가능), D < 0.4 (불균일)
- FR-12.4: **면 간 휘도 격차 (cross-surface balance)**
  - max(L_avg) / min(L_avg) ≤ 2.0 권장. 초과 시 "면 간 격차 큼 — 한 면의 빛이 다른 면을 압도" 경고.
- FR-12.5: **주변광 대비 (adaptation balance)**
  - 사용자가 입력한 주변광 휘도 L_amb (또는 룸 외부 조명 모드: dark/dim/normal)과 화면 평균 휘도 L_screen의 비율 검사
  - 권장: 0.05 ≤ L_amb / L_screen ≤ 0.25 (BBC/SMPTE 가이드라인)
  - 비율이 너무 낮으면 "주변광이 너무 어두워 눈 피로 / 잔상" 경고; 너무 높으면 "주변광이 화면을 묻음" 경고.
- FR-12.6: **종합 감상 품질 등급**
  - 5개 sub-score(시감 영역, 권장 대역, 균일도, 면 간 격차, 주변광 대비)에 가중 평균(기본 가중 25/25/20/15/15)으로 0–100 점 산출
  - A 85+, B 70–85, C 50–70, D < 50 로 매핑
  - 등급이 깎인 가장 큰 원인 1–2개를 한 줄 진단으로 표시 ("벽 평균 휘도 8 nit — 권장 대역 30 nit 대비 부족", "균일도 0.42 — 우측 벽이 어둡습니다")
- FR-12.7: **콘텐츠 타입 프리셋 + 사용자 정의 임계값**
  - 미디어아트/시네마/사이니지 프리셋 외에 사용자가 직접 권장 대역과 가중치를 조정할 수 있어야 함 (현장에 따라 기준이 다르므로)
- FR-12.8: 평가 결과를 시나리오 export(JSON/PDF)에 포함 — 클라이언트 보고서 용도
...
- **우측 결과 패널**:
  ...
  - **감상 품질 평가** — 종합 등급(A/B/C/D) 배지 + 5개 sub-score 막대 + 진단 메시지 1–2줄. 콘텐츠 프리셋 셀렉터(미디어아트/시네마/사이니지/사용자 정의), 주변광 휘도 입력
...
### 9.9 인체공학 시감 평가 모델
§9.4의 합산 휘도 L_total(P)로부터 면별 통계를 낸 뒤, 다음 다섯 항목의 sub-score를 0–100으로 산출하고 가중 평균하여 종합 등급을 계산한다.

#### (a) 시감 영역 (Photopic / Mesopic / Scotopic)
면 평균 휘도 L_avg(nit)에 대해

```
score_photopic =
  100,                                     if L_avg ≥ 3
  100 · (L_avg / 3),                       if 0.1 ≤ L_avg < 3   (mesopic 상단)
  20 · (L_avg / 0.1),                      if 0.005 ≤ L_avg < 0.1  (mesopic 하단)
  0,                                        if L_avg < 0.005    (scotopic, 색 인지 불가)
```

#### (b) 권장 휘도 대역 (target band)
콘텐츠 프리셋의 [L_low, L_high] 대역에 대해

```
            { 100,                          if L_low ≤ L_avg ≤ L_high
score_band={ 100 · (L_avg / L_low)^0.5,     if L_avg < L_low (under)
            { 100 · (L_high / L_avg)^0.7,   if L_avg > L_high (over, 눈부심 가중)
```

지수는 under보다 over에 더 큰 페널티(눈부심·피로). 모든 score는 [0, 100]으로 클램프.
...
score_uniformity =
  100, if U ≥ 0.75
  100 · (U − 0.4) / 0.35, if 0.4 ≤ U < 0.75  (선형 매핑)
  0,   if U < 0.4
```

#### (d) 면 간 격차 (Cross-surface balance)
활성 면들의 평균 휘도 집합 {L_avg^(i)}에 대해 비율 R = max / min,

```
score_balance =
  100, if R ≤ 2.0
  100 · (1 − (R − 2) / 3), if 2 < R ≤ 5
  0,   if R > 5
```

#### (e) 주변광 대비 (Adaptation balance)
주변광 L_amb(nit)와 화면 평균 L_screen에 대해 r = L_amb / L_screen,

```
score_adapt =
  100,                            if 0.05 ≤ r ≤ 0.25  (권장 대역)
  100 · (r / 0.05)^0.7,           if r < 0.05 (너무 어두운 주변)
  100 · (0.25 / r)^0.7,           if r > 0.25 (너무 밝은 주변)
```

#### (f) 종합 점수 / 등급
기본 가중치 w = (0.25, 0.25, 0.20, 0.15, 0.15)로 (a)–(e)를 가중 합산:

```
Q = Σ w_i · score_i           [0..100]
...
가장 점수가 낮은 1–2개 sub-score를 자연어 진단 메시지로 변환(템플릿 매칭). 예시:
- score_band 낮음 + L_avg < L_low → "면 X의 평균 휘도 12 nit — 권장 대역 30–100 nit 대비 부족 (목표 +18 nit)"
- score_uniformity 낮음 → "면 Y 균일도 0.42 — 한쪽 모서리가 어둡습니다 (프로젝터 위치/줌 조정 권장)"

#### 가정과 한계
- 색재현·색역은 미고려 (밝기 평가만)
- 동공 크기 변화, 안과 적응 시간(2–10분)은 미반영 — 정적 평가만
- 콘텐츠 자체의 평균 화소 휘도(APL)는 1.0(전백) 가정. 실제 콘텐츠는 평균 0.3–0.5 정도라 체감 휘도는 더 어두울 수 있음 (v2에서 APL 보정 옵션)
- 권장 대역은 학회/표준(SMPTE, BBC)과 미디어아트 현장 경험치를 절충한 휴리스틱이므로, 사용자 정의 임계값으로 오버라이드 가능 (FR-12.7)
...
│   ├── perception.ts   # §9.9 시감 sub-score, 종합 등급, 진단 메시지
│   └── stats.ts        # 균일도, 겹침, 차폐
...
interface Scenario {
  ...
  perceptual: {
    contentPreset: 'mediaArt' | 'cinema' | 'signage' | 'custom';
    targetBand: { low: number; high: number };  // nit
    ambientLuminance: number;                   // nit (주변광)
    weights: { photopic: number; band: number; uniformity: number; balance: number; adapt: number };
  };
}
...
const DEFAULTS = {
  ...
  perceptual: {
    contentPreset: 'mediaArt' as const,
    targetBand: { low: 30, high: 100 },         // nit, media art gallery
    ambientLuminance: 1.5,                      // nit (어두운 갤러리 가정)
    weights: { photopic: 0.25, band: 0.25, uniformity: 0.20, balance: 0.15, adapt: 0.15 },
  },
};
```
...
| M6.5. 인체공학 시감 평가 모듈 | 2 | §9.9 sub-score 5종, 종합 등급 + 진단 메시지, 콘텐츠 프리셋, 우측 패널 평가 카드, 단위 테스트 |
| M7. 저장/공유/Export | 2 | JSON import/export(관객·평가 설정 포함), URL 공유, PNG 캡처, CSV 관객 import |
| M8. 베타 테스트 + 4면 시나리오 검증 | 3 | 사용자(곽 대표) 실제 시나리오 dry run — 5대 + 30명 관객 부하/정합 검증, 평가 등급의 현장 체감과 비교 |
| M9. v1.0 릴리스 | 1 | README, 사용자 가이드, 데모 시나리오 3종(빈 공간 / 5명 / 30명) |

총 ≈ **28영업일 (약 5.5–6주)** — 1인 풀타임 기준. 파트타임이면 11–13주. 관객·차폐·시감 평가 기능 추가로 M6, M6.5, M8 비중이 증가했다.
...
| 시감 평가 휴리스틱이 현장 체감과 어긋남 | 의사결정 품질 저하 | UI에 "휴리스틱이며 콘텐츠 APL/색재현은 미고려"임을 명시; 임계값을 사용자 정의 가능; M8에서 곽 대표 실제 셋업의 평가 등급과 현장 체감을 비교해 가중치 보정 |
| 콘텐츠의 평균 화소 휘도(APL)가 1.0 가정 | 어두운 콘텐츠에서 과대평가 | v1은 안내 문구로 한계 명시; v2에서 APL 보정(0.3/0.5/사용자 입력) 옵션 도입 |
...
- 시감 평가 v2 — 콘텐츠 APL 보정, 색재현(색역/감마), 동적 적응 시간 모델, ICTCp 기반 HDR 평가
...
- O-10: 시감 평가의 콘텐츠 프리셋 기본 — "미디어아트 갤러리(30–100 nit, 주변광 1.5 nit)"로 가도 되는지 (현재 PRD 기본값)
- O-11: 평가 가중치 — 기본 (시감 영역 25 / 권장 대역 25 / 균일도 20 / 면 간 격차 15 / 주변광 15)을 그대로 갈지, 곽 대표 현장 경험에 맞춰 조정할지
- O-12: 주변광 입력을 단순 4단계 프리셋(완전 암실 0.1 / 어두움 1.5 / 보통 5 / 밝음 30 nit)으로 받을지, nit 직접 입력으로만 받을지
